...
def parse_mission_status_record(record: dict[str, Any]) -> dict[str, Any]:
...
def _extract_header_from_sns_notification(notification: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "eventType": _get_attribute_value(attributes.get("eventType")),
        "correlationId": _get_attribute_value(attributes.get("correlationId")),
        "sentAt": notification.get("Timestamp"),
        "version": _get_attribute_value(attributes.get("version"))
        or _get_attribute_value(attributes.get("schemaVersion"))
        or "1",
        "topicArn": topic_arn,
...
def _infer_channel(topic_arn: Any) -> str | None:
    if not isinstance(topic_arn, str):
        return None
    if "mission-status-changed-v1" in topic_arn:
        return "mission.status.changed.v1"
    if "rescue-mission-status-changed" in topic_arn:
        return "mission.status.changed.v1"
    return None
...
def _get_attribute_value(attribute: Any) -> Any:
    if isinstance(attribute, dict):
        return attribute.get("Value") or attribute.get("StringValue") or attribute.get("stringValue")
    return attribute
