...
def handler(event, context):
    ...
    logger.info(
        "Prioritization SQS batch received",
        extra={
            "extra_data": {
    ...
    for index, record in enumerate(records):
        ...
        logger.info(
            "Received prioritization SQS record",
            extra={
                "extra_data": {
                    "awsRequestId": aws_request_id,
                    "recordIndex": index,
        ...
        try:
            ...
            logger.info(
                ...
                extra={
                    "extra_data": {
                        ...
                        "evaluateId": body.get("evaluateId"),
                    }
                },
            )
            result = ingest_rescue_request_evaluation.execute(message)
            logger.info(
                "Ingested prioritization message",
                extra={
                    "extra_data": {
                        "awsRequestId": aws_request_id,
                        "recordIndex": index,
            ...
            logger.exception(
                "Failed to ingest RescueRequestEvaluatedEvent",
                extra={
                    "extra_data": {
                        "awsRequestId": aws_request_id,
                        "recordIndex": index,
    ...
    logger.info(
        "Prioritization SQS batch completed",
        extra={
            "extra_data": {
                "awsRequestId": aws_request_id,
                "recordCount": len(records),
