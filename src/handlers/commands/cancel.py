from src.application.services.event_publisher import publish_cancelled, publish_status_changed
...
def handler(event, context):
    try:
        ...
        if idempotency_key:
            replay = check_and_reserve(idempotency_key, "Cancel", body)
        ...
        try:
            ...
            if idempotency_key:
                finalize_failure(idempotency_key, "CANCEL_FAILED", str(e))
            raise
        ...
        try:
            ...
            publish_cancelled(request_id, result["eventId"], body.get("reason", ""))
        except Exception:
            pass
